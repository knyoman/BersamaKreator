...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const orderFilters = isSME
        ? { smeId: user?.id }
        : { influencerId: userProfile?.influencer_id || userProfile?.id };

      const { data, error: ordersError } = await getOrders(orderFilters);

      if (ordersError) throw ordersError;

      setOrders(data || []);
    } catch (err) {
      logger.error('[Dashboard] Error fetching data:', err.message);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container max-w-6xl mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Selamat datang kembali, {userProfile?.name}!</h1>
          <p className="text-gray-600">
            {isSME && 'Kelola kampanye dan pesanan Anda'}
            {isInfluencer && 'Kelola kampanye Anda'}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900 capitalize">{userProfile?.user_type === 'sme' ? 'Pemilik Bisnis' : 'Influencer'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">{isSME ? 'Pesanan Anda' : 'Kampanye Anda'}</h2>

          {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4">{error}</div>}

          {orders.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 mb-4">{isSME ? 'Belum ada pesanan' : 'Belum ada kampanye'}</p>
              {isSME && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-3 px-4 text-sm font-semibold text-gray-900">Nama Kampanye</th>
        ...
        {import.meta.env.DEV && (
          <div className="mt-8 bg-gray-100 rounded-lg p-4 text-xs text-gray-600">
            <p className="font-semibold mb-2">Info Debug (Development Only)</p>
            <p>User ID: {user?.id}</p>
            <p>Email: {user?.email}</p>
            <p>Tipe User: {userProfile?.user_type}</p>
            <p>Apakah UMKM: {isSME ? 'Ya' : 'Tidak'}</p>
            <p>Apakah Influencer: {isInfluencer ? 'Ya' : 'Tidak'}</p>
          </div>
        )}
      </div>
    </div>
