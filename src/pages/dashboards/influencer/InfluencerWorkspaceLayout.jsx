...
const baseNavItems = [
  { to: '/dashboard/overview', label: 'Ringkasan', icon: faGaugeHigh },
  { to: '/dashboard/performance', label: 'Analisis', icon: faChartLine },
  { to: '/dashboard/ai-assistant', label: 'Asisten AI', icon: faMagicWandSparkles },
  { to: '/dashboard/portfolio', label: 'Portofolio', icon: faBriefcase },
  { to: '/dashboard/pricing', label: 'Paket Harga', icon: faTags },
  { to: '/dashboard/availability', label: 'Ketersediaan', icon: faCalendarAlt },
  { to: '/dashboard/campaigns', label: 'Promosi', icon: faChartLine },
  { to: '/dashboard/earnings', label: 'Penghasilan', icon: faWallet },
  { to: '/dashboard/reviews', label: 'Ulasan', icon: faStar },
];
...
const SidebarContent = ({ userProfile, onNavigate, onLogout }) => {
  const navItems = [
    ...
    ...(userProfile?.username
      ? [{ to: `/influencer/${userProfile.username}`, label: 'Profil Publik', icon: faEye }]
      : []),
  ];
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950 text-white">
      <div className="px-4 py-4 border-b border-white/10">
        ...
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase text-gray-400">Ruang Kerja Influencer</p>
          <p className="text-sm font-bold text-white mt-1 truncate">{userProfile?.name || 'Kreator'}</p>
          <p className="text-xs text-gray-400 truncate">@{userProfile?.username || 'username-belum-diisi'}</p>
        </div>
      </div>
...
const InfluencerWorkspaceLayout = ({ userProfile, children }) => {
...
export default InfluencerWorkspaceLayout;
