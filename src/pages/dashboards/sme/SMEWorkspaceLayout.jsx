...
const smeNavItems = [
  ...
  { to: '/dashboard/campaigns', label: 'Promosi Saya', icon: faBullhorn },
  { to: '/dashboard/influencers', label: 'Cari Influencer', icon: faSearch },
  { to: '/dashboard/shortlist', label: 'Daftar Favorit', icon: faBookmark },
  { to: '/dashboard/ai-assistant', label: 'Asisten AI', icon: faMagicWandSparkles },
  { to: '/dashboard/payments', label: 'Pembayaran', icon: faWallet },
  { to: '/dashboard/reviews', label: 'Ulasan', icon: faStar },
  { to: '/dashboard/insights', label: 'Analisis', icon: faChartLine },
  { to: '/dashboard/business-profile', label: 'Profil Bisnis', icon: faBriefcase },
];
...
const SidebarContent = ({ userProfile, onNavigate, onLogout }) => (
  <div className="h-full flex flex-col bg-gray-950 text-white">
    <div className="px-4 py-4 border-b border-white/10">
      ...
      <div className="mt-4">
        <p className="text-xs font-semibold uppercase text-gray-400">Ruang Kerja UMKM</p>
        <p className="text-sm font-bold text-white mt-1 truncate">{userProfile?.name || 'Bisnis Anda'}</p>
        <p className="text-xs text-gray-400 truncate">{userProfile?.email || 'Kelola promosi bisnis'}</p>
      </div>
    </div>
...
const SMEWorkspaceLayout = ({ userProfile, children }) => {
      ...
      <div className="lg:col-start-2 min-w-0">
        <header className="lg:hidden sticky top-0 z-40 bg-white border-b border-gray-200">
          <div className="h-14 px-3 flex items-center justify-between">
            ...
            <FontAwesomeIcon icon={faUserTie} className="w-9 text-gray-500" />
          </div>
        </header>
...
export default SMEWorkspaceLayout;
